...
all_temp_data %>% 
  ...
  mutate(Season = case_when(Datetime_EST < "2019-05-15 00:00:00" ~ "Dry",
                            Datetime_EST >= "2019-05-15 00:00:00" ~ "Wet")) %>% 
...
all_temp_data %>% 
  ...
  mutate(Season = case_when(Datetime_EST < "2020-05-15 00:00:00" ~ "Dry",
                            Datetime_EST >= "2020-05-15 00:00:00" ~ "Wet")) %>% 
  dplyr::group_by(Time_of_day, site, Season) %>%
  dplyr::summarise(mean_temp = mean(Temperature), sd_temp = sd(Temperature), min_temp = min(Temperature), max_temp = max(Temperature))
...
all_temp_data %>% 
  ...
  mutate(Season = case_when(Datetime_EST < "2021-05-15 00:00:00" ~ "Dry",
                            Datetime_EST >= "2021-05-15 00:00:00" ~ "Wet")) %>% 
  dplyr::group_by(Time_of_day, site, Season) %>%
  dplyr::summarise(mean_temp = mean(Temperature), sd_temp = sd(Temperature), min_temp = min(Temperature), max_temp = max(Temperature)) 
...
filtered_data %>% 
  filter(Datetime_EST < "2021-12-08 00:00:00" & Datetime_EST >= "2020-10-13 00:00:00") %>% 
  full_join(., filtered_cures_data) %>% 
  mutate(Season = case_when(Datetime_EST < "2020-10-15 00:00:00" ~ "Wet", 
                            Datetime_EST <= "2021-05-26 00:00:00" & Datetime_EST >= "2020-10-15 00:00:00" ~ "Dry",
                            Datetime_EST > "2021-05-26 00:00:00" & Datetime_EST < "2021-10-15 00:00:00" ~ "Wet",
                            Datetime_EST >= "2021-10-15 00:00:00" ~ "Dry")) %>% 
  ungroup() %>% 
...
library(ncdf4)
  ...
  Climatology_MMM <- "raw_data/ct5km_climatology_v3.1.nc"
...
Long.data.DHW$DHW<-rollapplyr(Long.data.DHW$Weeks_Stress,list(-(83:0)),sum,fill=NA)
...
# make a new radar plot with DHW
