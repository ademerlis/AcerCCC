...
map04915_ko_ids <- unlist(strsplit(KEGG_enrich_CvN_categories$geneID[KEGG_enrich_CvN_categories$ID == "map04915"], "/"))

KEGG_enrich_CvN_categories %>% 
  filter(ID == "map04915" | ID == "map04916" | ID == "map04928") %>% 
...
# Brown2 WGCNA module - Nursery vs CCC DGEs with log-2-fold change list
...
```{r}
load("RData_files/Location_CCC_vs_nursery_fc_lfcshrink.RData") 
brown2_genes <- read_csv("WGCNA/brown2_genelist.csv") #482 genes 
...
```{r}
length(unique(KEGG_enrich_brown2@result$Description)) #245 unique KEGG terms
...
length(unique(KEGG_enrich_brown2_categories$Category)) #43 unique categories
```
...
# magenta3 WGCNA module - Nursery vs CCC DGEs with log-2-fold change list
