...
nurseryvsCCC_DGEs <- read_csv("results/Location_nursery_vs_CCC_annotDGEs_padj05.csv") #828 genes 

Location_nursery_vs_CCC.fc %>% right_join(kegg.list, by = "gene") %>% 
  right_join(., nurseryvsCCC_DGEs, by = "gene") %>%  #narrow down to only the DGEs
  select(lfc, KO) %>% 
  na.omit() %>% #414 genes
  distinct(KO, .keep_all = TRUE) %>% #367 distinct KEGG terms
  column_to_rownames(var = "KO") -> nurseryvsCCC_KEGG_DGEs
```
...
KEGG_enrich_TvI <- enrichKEGG(
                      gene = rownames(nurseryvsCCC_KEGG_DGEs),
                      organism = "ko",
                      keyType = "kegg",
                      pvalueCutoff = 0.05,
                      pAdjustMethod = "fdr",
...
```{r}
length(unique(KEGG_enrich_TvI@result$Description)) #284 unique KEGG terms
...
length(unique(KEGG_enrich_TvI_categories$Category)) #44 unique categories
```
...
str(filtered_KEGG_enrich_TvI) #200 KEGG terms
length(unique(filtered_KEGG_enrich_TvI$Category)) #33 categories
