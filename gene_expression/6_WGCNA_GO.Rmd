...
GO_slim_edited %>%
  full_join(GO_results, by = c("module", "ChildTerm" = "GO.ID", "Ontology")) %>% 
...
# bubble plots for significant modules
```{r}
merged_results %>% 
  filter(module == "darkmagenta" | module == "mediumpurple3") -> sig_wgcna_modules
...
sig_wgcna_modules %>% 
  ...
  dplyr::select(GO_slim_term:Ontology) %>% 
  distinct() %>% 
ggplot(., aes(x = fct_reorder(GO_slim_term_description, Count), y = Count, fill = Ontology)) +
  ...
  coord_flip()
#ggsave("4_WGCNA/GO_slim_barplot_sig_modules.pdf", width = 10, height = 6)
```
