...
for(mod in modules) {
  ...
  BP_Res <- GenTable(GOdata_BP, weightFisher = resultTopGO.BP,  topNodes = length(allBPGO))
  BP_Res$Class = "Biological_Process"
  BP_Res$GeneList <- vapply(BP_Res$GO.ID, function(go) {
    str_c(BP_ANNOTATION[[go]], collapse=",")
  }, character(1))
  
  CC_Res <- GenTable(GOdata_CC, weightFisher = resultTopGO.CC,  topNodes =  length(allCCGO))
  CC_Res$Class = "Cellular_Component"
  CC_Res$GeneList <- vapply(CC_Res$GO.ID, function(go) {
    str_c(CC_ANNOTATION[[go]], collapse=",")
  }, character(1))

  MF_Res <- GenTable(GOdata_MF, weightFisher = resultTopGO.MF,  topNodes =  length(allMFGO))
